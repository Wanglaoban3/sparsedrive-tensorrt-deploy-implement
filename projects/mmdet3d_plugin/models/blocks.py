...
class DeformableFeatureAggregation(BaseModule):
    ...
    def project_points(key_points, projection_mat, image_wh=None):
        ...
        x = key_points[..., 0].unsqueeze(1)
        y = key_points[..., 1].unsqueeze(1)
        z = key_points[..., 2].unsqueeze(1)

        p00 = projection_mat[:, :, 0, 0].unsqueeze(-1).unsqueeze(-1)
        p01 = projection_mat[:, :, 0, 1].unsqueeze(-1).unsqueeze(-1)
        p02 = projection_mat[:, :, 0, 2].unsqueeze(-1).unsqueeze(-1)
        p03 = projection_mat[:, :, 0, 3].unsqueeze(-1).unsqueeze(-1)

        p10 = projection_mat[:, :, 1, 0].unsqueeze(-1).unsqueeze(-1)
        p11 = projection_mat[:, :, 1, 1].unsqueeze(-1).unsqueeze(-1)
        p12 = projection_mat[:, :, 1, 2].unsqueeze(-1).unsqueeze(-1)
        p13 = projection_mat[:, :, 1, 3].unsqueeze(-1).unsqueeze(-1)

        p20 = projection_mat[:, :, 2, 0].unsqueeze(-1).unsqueeze(-1)
        p21 = projection_mat[:, :, 2, 1].unsqueeze(-1).unsqueeze(-1)
        p22 = projection_mat[:, :, 2, 2].unsqueeze(-1).unsqueeze(-1)
        p23 = projection_mat[:, :, 2, 3].unsqueeze(-1).unsqueeze(-1)

        u = p00 * x + p01 * y + p02 * z + p03
        v = p10 * x + p11 * y + p12 * z + p13
        depth = torch.clamp(p20 * x + p21 * y + p22 * z + p23, min=1e-5)

        points_2d = torch.stack((u / depth, v / depth), dim=-1)
