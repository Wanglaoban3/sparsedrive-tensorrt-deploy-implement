...
class SparsePoint3DKeyPointsGenerator(BaseModule): 
    def __init__(
        ...
        self.num_heights = len(fix_height)
        self.num_pts = num_sample * self.num_heights * num_learnable_pts
        ...
        sample_idx = torch.arange(num_sample, dtype=torch.long).view(
            num_sample, 1, 1
        )
        sample_idx = sample_idx.expand(
            num_sample, self.num_heights, num_learnable_pts
        ).reshape(self.num_pts)
        z_flat = z_vals.view(1, self.num_heights, 1).expand(
            num_sample, self.num_heights, num_learnable_pts
        ).reshape(self.num_pts)

        self.register_buffer('z_vals', z_vals)
        self.register_buffer('sample_idx', sample_idx)
        self.register_buffer('z_flat', z_flat)
    ...
    def forward(
        ...
        base_xy = base_xy.index_select(2, self.sample_idx)

        offset = self.learnable_fc(instance_feature).view(
            bs, num_anchor, self.num_pts, 2
        )
        xy = base_xy + offset
        z = xy[..., :1] * 0.0 + self.z_flat.to(dtype=xy.dtype).view(
            1, 1, self.num_pts, 1
        )
        key_points = torch.cat([xy, z], dim=-1)
